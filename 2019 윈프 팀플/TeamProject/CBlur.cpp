...
IMPLEMENT_DYNAMIC(CBlur, CDialogEx)

//CBlur::CBlur(CWnd* pParent /*=nullptr*/)
//	: CDialogEx(IDD_DIALOG_CUTPREV, pParent)
//{
//
//}

CBlur::CBlur(CAdvImg* img, CRgn* rgn, CWnd* pParent)
	:CDialogEx(IDD_DIALOG_BLUR, pParent)
{
...
void CBlur::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CBlur, CDialogEx)
	//ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_ALPHA, &CBlur::OnNMCustomdrawSliderAlpha)
	//ON_BN_CLICKED(IDC_CHECK_BLUR, &CBlur::OnBnClickedCheckBlur)
	ON_BN_CLICKED(IDC_APPLYBT, &CBlur::OnBnClickedApplybt)
	//ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_HUE, &CBlur::OnNMCustomdrawSliderHue)
	//ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_BETA, &CBlur::OnNMCustomdrawSliderBeta)
END_MESSAGE_MAP()
...
BOOL CBlur::PreCreateWindow(CREATESTRUCT& cs)
{
...
BOOL CBlur::OnInitDialog()
{
	...
	m_sliderBlur.SetRange(0, 20, TRUE);
...
/*void CBlur::OnNMCustomdrawSliderAlpha(NMHDR* pNMHDR, LRESULT* pResult)
{
...
/*void CBlur::OnBnClickedCheckBlur()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}*/


void CBlur::OnBnClickedApplybt()
{
	setPreviewImg();
}


/*void CBlur::OnNMCustomdrawSliderHue(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;
...
void CBlur::OnNMCustomdrawSliderBeta(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;
