...
IMPLEMENT_DYNAMIC(CGray, CDialogEx)

//CGray::CGray(CWnd* pParent /*=nullptr*/)
//	: CDialogEx(IDD_DIALOG_CUTPREV, pParent)
//{
//
//}

CGray::CGray(CAdvImg* img, CRgn* rgn, CWnd* pParent)
	:CDialogEx(IDD_DIALOG_GRAY, pParent)
{
...
BEGIN_MESSAGE_MAP(CGray, CDialogEx)
	//ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_ALPHA, &CGray::OnNMCustomdrawSliderAlpha)
	//ON_BN_CLICKED(IDC_CHECK_BLUR, &CGray::OnBnClickedCheckBlur)
	ON_BN_CLICKED(IDC_APPLYBT, &CGray::OnBnClickedApplybt)
	//ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_HUE, &CGray::OnNMCustomdrawSliderHue)
	//ON_NOTIFY(NM_CUSTOMDRAW, IDC_SLIDER_BETA, &CGray::OnNMCustomdrawSliderBeta)
END_MESSAGE_MAP()
...
BOOL CGray::PreCreateWindow(CREATESTRUCT& cs)
{
...
/*void CGray::OnNMCustomdrawSliderAlpha(NMHDR* pNMHDR, LRESULT* pResult)
{
...
/*void CGray::OnBnClickedCheckBlur()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}*/


void CGray::OnBnClickedApplybt()
{
	setPreviewImg();
}


/*void CGray::OnNMCustomdrawSliderHue(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;
...
void CGray::OnNMCustomdrawSliderBeta(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	*pResult = 0;
